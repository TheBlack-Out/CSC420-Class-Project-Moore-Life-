...
public class UserRegistered extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String msg = " ";
            if (i != 0) {
                msg = "<br />" + "New userkey record has also been created";
                pw.println("<font size='5' color=blue>" + msg + "</font>");
        ...
        try {
            Class.forName(driver).newInstance();
            conn = DriverManager.getConnection(url + dbName, "root", "TOl0Pok#4");
            PreparedStatement pst = (PreparedStatement) conn.prepareStatement("insert into Levelkey values (?,?)");
            ...
            String msg = " ";
            if (i != 0) {
                msg = "<br />" + "New levelkey record has also been created";
                pw.println("<font size='5' color=blue>" + msg + "</font>");

            } else {
                msg = "<br />" + "Failed to insert levelkey record";
                pw.println("<font size='5' color=red>" + msg + "</font>");
            }
            pst.close();
        } catch (Exception e) {
